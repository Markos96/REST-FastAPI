...
def create_user(users: User, db: Session = Depends(get_db), token: str = Depends(verify_token)):
    return user.add_user(users, db)


@router.get("/get", response_model=UserDetails, status_code=status.HTTP_200_OK)
def get_by_user_id(user_id: int, db: Session = Depends(get_db), token: str = Depends(verify_token)):
...
def update_user(userUpdate: UserUpdate, user_id: int, db: Session = Depends(get_db), token: str = Depends(verify_token)):
